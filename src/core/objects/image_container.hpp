...
public:
     ...
     * 
     * Working Process:
     *   1. If object is marked non-selectable (!isSelectable), returns false immediately.
     *   2. Broad-Phase: If point is outside the cached axis-aligned bounding box (bounds), returns false.
     *   3. Fast-Path: If object is already selected (`isSelected == 1`), returns true immediately.
     *      This avoids expensive inverse affine matrix calculations and ensures buttery-smooth
     *      drag/move interactions.
     *   4. Narrow-Phase: Inverts the 2D affine transform and projects the world-space point
     *      into the image's unrotated local rectangle space.
     * 
     * @param hitWorldX World X coordinate in mm.
     * @param hitWorldY World Y coordinate in mm.
     * @return true if the point falls inside the image.
     */
